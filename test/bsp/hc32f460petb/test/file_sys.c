...
UINT32	CH376ReadVar32( uint8_t var )  /* 读CH376芯片内部的32位变量 */
{
...
void	CH376WriteVar32( uint8_t var, UINT32 dat )  /* 写CH376芯片内部的32位变量 */
{
	xWriteCH376Cmd( CMD50_WRITE_VAR32 );
	xWriteCH376Data( var );
	xWriteCH376Data( (uint8_t)dat );
	xWriteCH376Data( (uint8_t)( (UINT16)dat >> 8 ) );
...
uint8_t	CH376ByteLocate( UINT32 offset )  /* 以字节为单位移动当前文件指针 */
{
	xWriteCH376Cmd( CMD4H_BYTE_LOCATE );
	xWriteCH376Data( (uint8_t)offset );
	xWriteCH376Data( (uint8_t)((UINT16)offset>>8) );
...
uint8_t	CH376ByteRead( uint8_t * buf, UINT16 ReqCount, PUINT16 RealCount )  /* 以字节为单位从当前位置读取数据块 */
{
	uint8_t	s;
...
uint8_t	CH376ByteWrite( uint8_t * buf, UINT16 ReqCount, PUINT16 RealCount )  /* 以字节为单位向当前位置写入数据块 */
{
	uint8_t	s;
...
uint8_t	CH376DiskCapacity( PUINT32 DiskCap )  /* 查询磁盘物理容量,扇区数 */
{
	uint8_t	s;
...
uint8_t	CH376DiskQuery( PUINT32 DiskFre )  /* 查询磁盘剩余空间信息,扇区数 */
{
	uint8_t	s;
	uint8_t	c0, c1, c2, c3;
...
uint8_t	CH376SecLocate( UINT32 offset )  /* 以扇区为单位移动当前文件指针 */
{
...
uint8_t	CH376DiskReadSec( uint8_t * buf, UINT32 iLbaStart, uint8_t iSectorCount )  /* 从U盘读取多个扇区的数据块到缓冲区,不支持SD卡 */
...
uint8_t	CH376DiskWriteSec( uint8_t * buf, UINT32 iLbaStart, uint8_t iSectorCount )  /* 将缓冲区中的多个扇区的数据块写入U盘,不支持SD卡 */
...
uint8_t	CH376LongNameWrite( uint8_t * buf, UINT16 ReqCount )  /* 长文件名专用的字节写子程序 */
{
	uint8_t	s, c;
...
{
	uint8_t	s, i;
	uint8_t	DirBlockCnt;	/* 长文件名占用文件目录结构的个数 */
