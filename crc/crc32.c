...
uint32_t al_crc32(uint32_t crc, const void *p, uint32_t len)
{
     return al_crc32_no_comp(crc ^ 0xffffffffL, p, len) ^ 0xffffffffL;
}
					 ...
					 uint8_t *output, uint32_t chunk_sz)
{
	...
	crc = al_crc32_wd(0, input, ilen, chunk_sz);
	crc = htonl(crc);
