...
#ifndef __AL_BITOPTS_BITMAP_H
#define __AL_BITOPTS_BITMAP_H 1
...
#include "alumy/bitops/hweight.h"
#include "alumy/find.h"
...
#define bitmap_size(nbits)	(AL_ALIGN(nbits, BITS_PER_LONG) / BITS_PER_BYTE)
...
bool bitmap_equal(const unsigned long *src1, const unsigned long *src2, unsigned int nbits)
{
	...
	if (__builtin_constant_p(nbits & BITMAP_MEM_MASK) &&
        AL_IS_ALIGNED(nbits, BITMAP_MEM_ALIGNMENT))
...
void bitmap_set(unsigned long *map, unsigned int start, unsigned int nbits)
{
	if (__builtin_constant_p(nbits) && nbits == 1)
		...
		*map |= GENMASK(start + nbits - 1, start);
         ...
         AL_IS_ALIGNED(start, BITMAP_MEM_ALIGNMENT) &&
